...
def get_empty_hist(name, title):
	hist = ROOT.TH1D(name, title, 25, 70, 170)
	return hist
...
def get_hist(path2input, filename, lep, data = 0):
	...
	for event in range(0, n_events):
		...
		ep2.SetPtEtaPhiM(lep2_pt,lep2_eta,lep2_phi,lep2_mass)
		bp = bp1 + bp2
		ep = ep1 + ep2
		bb_pt = bp.Pt()
		ee_pt = ep.Pt()
...
#                hist.Fill(H_mass[0],(Vtype[0] == lep_type)*(dEta_jj<1.6)*(dR_jj>0.5)*(dR_jj < 3.2)*weight[0]*(bjet2_PtReg[0]>30)*(bjet2_DeepCSV>0.2)*(bjet1_DeepCSV>0.5)*(lep2_pt < 130)*(lep1_pt > 40)*(ee_m > 87)*(ee_m < 95))
                        hist.Fill(H_mass[0],(dEta_jj<1.6)*(dR_jj>0.5)*(dR_jj < 3.2)*weight[0]*(bjet2_PtReg[0]>30)*(bjet2_DeepCSV>0.2)*(bjet1_DeepCSV>0.5)*(lep2_pt < 130)*(lep1_pt > 40)*(ee_m > 87)*(ee_m < 95))
                else:
                        hist.Fill(H_mass[0],(Vtype[0] == lep_type)*(dEta_jj<1.6)*(dR_jj>0.5)*(dR_jj < 3.2)*weight[0]*(bjet2_PtReg[0]>30)*(bjet2_DeepCSV>0.2)*(bjet1_DeepCSV>0.5)*(lep2_pt < 130)*(lep1_pt > 40)*(ee_m > 87)*(ee_m < 95))  
...
def draw(lep):
	...
	h_data.GetXaxis().SetTitle('m_{bb}, GeV')	
	h_data.Draw('PE')
        h_data.SetMinimum(0)
...
#        c1.SaveAs(lep+'_mbb_detajj.root')
        c1.SaveAs(lep+'_mbb_cuts.png')
	c2.cd()
	h_data_extrac_MC.Draw('PE')
        line=ROOT.TLine(70,0,170,0)
        line.SetLineColor(ROOT.kRed)
        line.Draw()
	c2.Update()
	c2.Modified()
        c2.SaveAs(lep+'_mbb_cuts_diff.png')
	text = raw_input()
